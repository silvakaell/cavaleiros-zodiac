...
// Mostra as localizações disponíveis e em breve.
...
function LocationOrnament({ loc }) {
  if (loc.id === "sanctuary") {
    // Círculo zodiacal com 12 pontos
    ...
    return (
      <svg viewBox="0 0 120 120" style={{ width: 120, height: 120, display: "block", margin: "0 auto 12px" }}>
        ...
        {pts.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="3" fill={loc.color} opacity="0.7"/>
        ))}
        {pts.map((p, i) => (
          <line key={i} x1={p.x} y1={p.y} x2="60" y2="60"
            stroke={loc.color} strokeWidth="0.4" opacity="0.15"/>
        ))}
...
function LocationCard({ loc, onSelect }) {
  ...
  return (
    <div
      style={{
        ...
        boxShadow: hovered && !locked
          ? `0 0 28px ${loc.color}28, inset 0 0 20px ${loc.color}08`
          : "none",
    ...
    >
      ...
      <div style={{
        height: "1px",
        background: locked ? "#0e1e2e" : "#1a2a3a",
        margin: "12px 0",
      }}/>
      ...
      <p style={{ ...S.cardHouses, color: locked ? "#1a2a3a" : loc.color + "99" }}>
        {loc.houseCount} {loc.houseCount === 12 ? "casas" : loc.houseCount === 7 ? "batalhas" : "etapas"}
      </p>
  ...
  return (
    <div style={S.outer}>
      ...
      <div style={S.page}>
        ...
        {/* Cabeçalho */}
        <div style={S.header}>
          <p style={S.eyebrow}>A TRAVESSIA</p>
          <h1 style={S.title}>Campo de Batalha</h1>
          <p style={S.sub}>Escolha onde sua saga começa</p>
        </div>
